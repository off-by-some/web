        ...
        <div class="hero__container">
            ...
            <section class="stats" aria-labelledby="stats-title">
                ...
                <div class="stats__grid" role="group" aria-label="Performance statistics">
                    {#each stats as stat, index}
                        <div class="stat" data-type={stat.type} role="img" aria-label={getStatDescription(stat)}>
                            <div
                                class="stat__number"
                            ...
                            <div class="stat__label">{@html stat.label}</div>
                        </div>
                    {/each}
                </div>
            </section>
            ...
            <section class="tech" aria-labelledby="tech-title">
                ...
                <div class="tech__grid" role="group" aria-label="Technology categories">
                    {#each techStackWithVisibleTags as category, index}
                        <button 
                        ...
                        >
                            ...
                            <div class="category__tags" bind:this={techContainers[index]}>
                                ...
                                {#if category.visibleTags.overflow > 0}
                                    <span class="tech__tag tech__tag--overflow" role="listitem" aria-label="{category.visibleTags.overflow} more technologies">+{category.visibleTags.overflow}</span>
                                {/if}
                            </div>
                            <span id="tech-category-{index}" class="sr-only">
                                Contains {category.technologies.length} technologies including {category.technologies.slice(0, 3).join(', ')}
...
.hero__actions {
    ...
    animation: actionsReveal var(--token-motion-duration-slow) var(--token-motion-ease-out) 0.8s both;
...
.tech__category {
    ...
    &::before {
        ...
        background: linear-gradient(
            135deg,
            var(--token-shadow-glow-subtle) 0%,
            transparent 60%
        );
        opacity: 0;
        transition: opacity var(--token-motion-duration-normal) var(--token-motion-ease-out);
